...
핵심 개선 사항
- 기존 다중 모델 비교 구조를 XGBoost 중심 학습 구조로 교체
- 날짜 분할(train / validation / test)은 유지하여 시계열 누수 방지
- year, month, day, weekday, weekofyear, season_code 등 날짜 파생변수 강화
- 주기 인코딩 + 비/습도/풍속 상호작용 변수 추가
- log1p 타깃 변환으로 극단값 영향 완화
- GridSearchCV + TimeSeriesSplit으로 XGBoost 하이퍼파라미터 탐색

실행 방법
1) 일반 파이썬 실행
   python merged_submission_date_weather_only_improved_xgb.py

2) Streamlit 예측 UI 실행
   streamlit run merged_submission_date_weather_only_improved_xgb.py
...
def make_features(daily_bike: pd.DataFrame, weather_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    df["season_code"] = df["season"].map(season_map).astype(int)

    (REFORMED_DIR / DATE_ORIGIN_FILE).write_text(
        str(pd.to_datetime(date_origin).date()),
        encoding="utf-8",
    )
...
def run_preprocessing():
    ...
    evaluate_xgboost_bundle.cache_clear()
    train_model_for_inference.cache_clear()
...
def validate_xgboost_installation():
    if XGBRegressor is None:
        raise ImportError(
            "xgboost 패키지가 설치되어 있지 않습니다. `pip install xgboost` 후 다시 실행하세요. "
            f"원본 오류: {XGBOOST_IMPORT_ERROR}"
        )
...
def evaluate_xgboost_bundle():
    ...
    best_model_name = "xgboost_gridsearch_log"
...
def run_profiling_report():
    ...
    profile = ProfileReport(
        df,
        title="Ddarungi Demand Prediction Profiling Report (XGBoost Date + Weather Model)",
        explorative=True,
    )
...
def run_streamlit_prediction_app():
    ...
    st.caption("날짜와 날씨 정보만 입력하면, XGBoost GridSearchCV로 튜닝된 모델로 예측합니다.")
    st.info(
        "이 버전은 lag/rolling 없이 날짜+날씨만 사용합니다. 대신 XGBoost, 날짜 파생변수 강화, 주기 인코딩, 상호작용 변수, GridSearchCV 튜닝을 적용했습니다."
    )
    ...
    if submitted:
        try:
            ...
            with st.expander("최적 하이퍼파라미터 보기", expanded=False):
            ...
            with st.expander("하이퍼파라미터 탐색 결과 보기", expanded=False):
            ...
            with st.expander("해석 시 주의점", expanded=False):
                st.markdown(
                    """
                    - 이 예측은 **날짜 + 날씨 기반 XGBoost 모델**입니다.
                    - lag/rolling 같은 과거 대여량 변수는 사용하지 않습니다.
                    - 성능 향상을 위해 날짜 파생변수, 주기 인코딩, 상호작용 변수, GridSearchCV 튜닝을 적용했습니다.
...
def main_cli():
    log("XGBoost 개선 버전 단일 제출용 파이썬 파일 실행 시작")
